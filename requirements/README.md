...
- both files include distribution hashes and are consumed with
  `pip --require-hashes`.
...
python -m pip install --no-deps dist/robotactile_benchmark-*.whl
```
...
dependencies change, maintainers update `uv.lock`, export both pip lock files,
and verify the pip-only CI job before merging. Rerun the local package install
after changing source files; the supported bootstrap deliberately avoids
depending on the pip version required by PEP 660 editable installs.
