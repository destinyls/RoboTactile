#!/usr/bin/env python3
"""Capture a measured N0 empty-gripper rest reference in one Isaac process."""
...
def main() -> int:
    ...
    if (
        base.policy_kind is not LivePolicyKind.N0
        or base.condition is not Condition.CLEAN
    ):
        raise ValueError("base request must be an N0 Clean request")
    ...
    request = replace(
        ...
        max_observation_steps=25,
        runtime_dir=(
            layout.runtime / "live-univtac" / "n0-rest-calibration" / base.task_id
        ),
        output_dir=live_output,
    )
    generated = write_official_n0_clean_request(args.request_output, request)
    ...
    backend_factory = partial(
        default_live_backend_factory,
        n0_action_execution_contract=N0_TRAINING_60HZ_ACTION_EXECUTION_CONTRACT,
        calibration_contract=N0_EMPTY_GRIPPER_CALIBRATION_CONTRACT,
    )
    binding = build_official_n0_live_binding(
        request,
        manifest=runtime.manifest,
        source_root=args.n0_source_root.expanduser().absolute(),
        host=args.n0_host,
        port=args.n0_port,
