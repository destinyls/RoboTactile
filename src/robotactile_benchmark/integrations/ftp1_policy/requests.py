"""Content-bound live request generation for the official FTP-1 policy."""
...
class GeneratedFTP1PolicyRequest:
...
def build_official_ftp1_policy_request(
    *,
    ...
    """Bind source, complete checkpoint tree, prompt, and fault identities."""
    ...
    system_id = f"official-ftp1.{manifest.task_id}.{manifest.checkpoint_revision[:16]}"
    ...
    request = LiveUniVTACRunRequest(
        ...
        policy_kind=LivePolicyKind.FTP1_POLICY,
        ...
        runtime_dir=(
            layout.runtime / "live-univtac" / "ftp1-policy" / manifest.task_id
        ),
        ...
        # These legacy-named fields are the current generic external-policy
        # identity slots. They bind FTP-1, not N0-TWAM, for this policy kind.
...
def write_official_ftp1_policy_request(
    request_path: Path,
    request: LiveUniVTACRunRequest,
    ...
    if request.policy_kind is not LivePolicyKind.FTP1_POLICY:
        raise ValueError("request must select policy_kind=ftp1_policy")
    ...
    return GeneratedFTP1PolicyRequest(
        request=request,
        request_path=target,
...
def _publish_canonical_json(target: Path, payload: bytes) -> None:
    if target.is_symlink():
        raise ValueError("FTP-1 request path cannot be a symlink")
    target.parent.mkdir(parents=True, exist_ok=True)
    if target.exists():
        if not target.is_file() or target.read_bytes() != payload:
            raise FileExistsError("refusing to replace a different FTP-1 request")
        return
    ...
    try:
        ...
        if not target.is_file() or target.read_bytes() != payload:
            raise FileExistsError(
                "concurrent FTP-1 request publication disagrees"
            ) from None
    finally:
